...
<script>
	...
	// Local filter state — sync from URL params on each navigation
	let showCode = '';
	let year = '';
	let dateFrom = '';
	let dateTo = '';
	...
	// Re-sync local state when data.filters changes (e.g. on navigation)
	$: if (data.filters) {
	...
	function applyFilters() {
		...
		goto(`${base}/shows/ticket_purchases?${buildParams(1)}`, { invalidateAll: true });
	}
	...
	function clearFilters() {
		...
		patronSearch = '';
		paymentMethod = '';
		goto(`${base}/shows/ticket_purchases`, { invalidateAll: true });
	}

	/** @param {number} page */
	function goToPage(page) {
		goto(`${base}/shows/ticket_purchases?${buildParams(page)}`, { invalidateAll: true });
	}

	$: hasFilters = showCode || year || dateFrom || dateTo || patronSearch || paymentMethod;
	...
	<div class="filter-card">
		...
		<div class="filter-grid">
			...
			<div class="filter-group">
				...
				<select id="filterYear" bind:value={year} on:change={() => { dateFrom = ''; dateTo = ''; applyFilters(); }}>
			...
			<div class="filter-group">
				...
				<input type="date" id="filterFrom" bind:value={dateFrom} on:change={() => { year = ''; applyFilters(); }} />
			</div>

			<div class="filter-group">
				<label for="filterTo">To Date</label>
				<input type="date" id="filterTo" bind:value={dateTo} on:change={() => { year = ''; applyFilters(); }} />
			</div>

			<div class="filter-group">
				<label for="filterPatron">Patron</label>
				<div class="search-row">
					<input type="text" id="filterPatron" bind:value={patronSearch} placeholder="Name or email..." on:keydown={(e) => e.key === 'Enter' && applyFilters()} />
					{#if patronSearch}
						<button class="btn-search" on:click={applyFilters}>Go</button>
					{/if}
				</div>
			</div>
			...
			<div class="filter-group">
				...
				<select id="filterPayment" bind:value={paymentMethod} on:change={applyFilters}>
		...
		{#if data.years.length > 0}
			<div class="quick-filters">
				...
				{#each data.years.slice(0, 5) as y (y)}
					<button class="quick-btn" class:active={year === y.toString()} on:click={() => setYearFilter(y)}>{y}</button>
				{/each}
			</div>
		{/if}
	</div>
