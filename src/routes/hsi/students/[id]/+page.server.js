...
export const load = async ({ params }) => {
  ...
  try {
    ...
    const student = {
      ...studentResult[0],
      account_date: studentResult[0].account_date
    ...
    const earliestReg = await sql`
      ...
      WHERE student_id = ${studentId}
    `;
    student.member_since = earliestReg[0].first_class_date
      ? earliestReg[0].first_class_date.toISOString().split('T')[0]
      : student.account_date;
    ...
    return {
      ...
      registrations: registrations.map(r => ({
        ...
        registration_date: r.registration_date ? r.registration_date.toISOString().split('T')[0] : null,
