...
<script>
	...
	function handleFileSelect(event) {
		...
		reader.onload = (e) => {
			parseCSV(/** @type {string} */ (e.target?.result));
		};
	...
	function parseCSV(text) {
		...
		const colMap = {
			...
			registration_date: findCol(header, [
				'last ticket bought',
				'last ticket',
				'ticket date',
				'registration date',
	...
	function removeCsvRow(index) {
		csvParsed = csvParsed.filter(
			(/** @type {any} */ _item, /** @type {number} */ i) => i !== index
		);
	}
</script>
				...
				<details class="results-details">
					...
					<ul>
						{#each form.results as r, i (i)}
							<li>
								{r.name} ({r.email}) —
								<span class="tag {r.status === 'new student' ? 'tag-new' : 'tag-existing'}"
									>{r.status}</span
								>
							</li>
						{/each}
							...
							<tr>
								...
								<td>
									<input
										...
										bind:value={row.registration_date}
										class="input-date"
									/>
								</td>
								<td>
		...
		<div class="csv-section">
				...
				<div class="config-row">
					...
					<div class="config-group config-group-wide">
						...
						<input
							...
							class="input-text"
							placeholder="e.g. Jan 2026 Level 1"
						/>
					</div>
				</div>
			</div>
			...
			{#if csvPreview && csvParsed.length > 0 && !reviewMode}
				...
				<form
					...
					use:enhance={() => {
						return async ({ result }) => {
							/** @type {any} */
							const resData = result.type === 'success' ? result.data : null;
							if (resData?.action === 'csv_check') {
								matchResults = resData.matchResults;
								reviewMode = true;

								// Handle session info from server
								if (resData.sessionInfo?.exists) {
									sessionInfo = resData.sessionInfo;
								...
								decisions = matchResults.map((m) => {
									...
									if (m.matchType === 'existing_no_reg') {
										...
										if (m.dbStudent) {
											if (
												m.csv.first_name.toLowerCase() !==
												(m.dbStudent.first_name || '').toLowerCase()
											)
												updateFields.push('first_name');
											if (
												m.csv.last_name.toLowerCase() !==
												(m.dbStudent.last_name || '').toLowerCase()
											)
												updateFields.push('last_name');
											if (m.csv.phone && m.csv.phone !== (m.dbStudent.phone || ''))
									...
									if (m.dbStudent) {
										if (
											...
											updateStudentFields.push('first_name');
										if (
											m.csv.last_name.toLowerCase() !== (m.dbStudent.last_name || '').toLowerCase()
										)
											updateStudentFields.push('last_name');
										if (m.csv.phone && m.csv.phone !== (m.dbStudent.phone || ''))
											updateStudentFields.push('phone');
									}
									if (m.dbRegistration) {
										if (csvClassDate !== m.dbRegistration.class_date)
											updateRegFields.push('class_date');
										if (m.csv.amount_paid !== m.dbRegistration.amount_paid)
											updateRegFields.push('amount_paid');
										if (
											m.csv.registration_date &&
											m.csv.registration_date !== m.dbRegistration.registration_date
										)
											updateRegFields.push('registration_date');
									}
									...
									<tr class:guest-row={row.is_guest}>
										...
										{#if row.is_guest}
											<td
												><input
													type="text"
													bind:value={csvParsed[i].first_name}
													class="input-text input-guest"
											...
											<td
												><input
													type="text"
													bind:value={csvParsed[i].last_name}
													class="input-text input-guest"
										...
										<td>
											{#if row.is_guest}
												<span class="guest-tag" title="Purchased by {row.purchaser_name}"
													>Guest</span
												>
											{/if}
											<button
			...
			{:else if reviewMode && matchResults.length > 0}
				...
				>
					...
					<div class="review-summary">
						<span class="review-badge badge-new"
							>{matchResults.filter((m) => m.matchType === 'new').length} new</span
						>
						<span class="review-badge badge-add"
							>{matchResults.filter((m) => m.matchType === 'existing_no_reg').length} existing (no reg)</span
						>
						<span class="review-badge badge-match"
							>{matchResults.filter((m) => m.matchType === 'existing_with_reg').length} existing (has
							reg)</span
						>
					</div>
						...
						{#each matchResults as match, i (i)}
							<div class="review-card {match.matchType}">
								<div class="review-header">
									...
									{:else if match.matchType === 'existing_no_reg'}
										<span class="match-badge badge-add"
											>Existing — No Registration for {csvClassCode}</span
										>
									{:else}
										<span class="match-badge badge-match"
											>Existing — Has {match.dbRegistration.class_code} Registration</span
										>
									{/if}
								</div>
								...
								{#if match.matchType === 'new'}
									<div class="review-action-row">
										<label class="radio-label">
											<input type="radio" bind:group={decisions[i].action} value="add_new" /> Add new
											...
											<div class="diff-table">
												{#if match.csv.first_name.toLowerCase() !== (match.dbStudent.first_name || '').toLowerCase()}
													<label class="diff-row">
														<input
															type="checkbox"
															bind:group={decisions[i].updateFields}
															value="first_name"
														/>
								...
								{:else}
									...
									<div class="review-action-row">
										<label class="radio-label">
											<input type="radio" bind:group={decisions[i].action} value="update_reg" /> Update
											existing registration
										</label>
										<label class="radio-label">
											<input type="radio" bind:group={decisions[i].action} value="add_reg" /> Add as new
											registration
										</label>
										<label class="radio-label">
											<input type="radio" bind:group={decisions[i].action} value="skip" /> Skip
											...
											<div class="diff-table">
												{#if match.csv.first_name.toLowerCase() !== (match.dbStudent.first_name || '').toLowerCase()}
													<label class="diff-row">
														<input
															type="checkbox"
															bind:group={decisions[i].updateStudentFields}
															value="first_name"
														/>
														<span class="diff-field">First Name:</span>
														<span class="diff-db">{match.dbStudent.first_name}</span>
											...
											<div class="diff-table">
												{#if csvClassDate !== match.dbRegistration.class_date}
													<label class="diff-row">
														<input
															type="checkbox"
															bind:group={decisions[i].updateRegFields}
															value="class_date"
														/>
												...
												{#if match.csv.amount_paid !== match.dbRegistration.amount_paid}
													<label class="diff-row">
														...
														<span class="diff-db"
															>${match.dbRegistration.amount_paid.toFixed(2)}</span
														>
														<span class="diff-arrow">→</span>
			...
			{:else if csvPreview}
				<div class="alert alert-error">
					No valid rows found in CSV. Make sure it has columns for email, first name, and last name.
				</div>
			{/if}
		</div>
	{/if}
