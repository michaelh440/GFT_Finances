<!-- src/routes/hsi/classes/new/+page.svelte -->
<script>
	...
	$: class_type = form?.values?.class_type ?? '';
	$: student_type = form?.values?.student_type ?? '';
	$: standard_price = form?.values?.standard_price ?? 0;
	$: track = form?.values?.track ?? '';
	$: vbo_event_id = form?.values?.vbo_event_id ?? '';
	$: description = form?.values?.description ?? '';
	$: duration_value = form?.values?.duration_value ?? '';
	$: duration_unit = form?.values?.duration_unit ?? '';
		...
		<form method="POST" use:enhance>
			<div class="form-grid">
				<div class="form-group">
					<label for="class_code">Class Code <span class="required">*</span></label>
					<input
						type="text"
						id="class_code"
						name="class_code"
						value={class_code}
						required
						placeholder="e.g. IMPROV101"
						style="text-transform: uppercase;"
					/>
					<span class="help-text">Unique identifier. Will be stored in uppercase.</span>
				</div>
				...
				<div class="form-group">
					...
					<input
						...
						name="vbo_event_id"
						value={vbo_event_id}
						placeholder="e.g. Level One: Intro to Improv"
					/>
					<span class="help-text">Event name as it appears in VBO CSV exports.</span>
				</div>
			...
			<div class="form-actions">
				<a href={resolve('/hsi/classes')} class="btn-secondary">Cancel</a>
