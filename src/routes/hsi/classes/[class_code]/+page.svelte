...
<div class="container">
	...
	{:else}
		<header>
			<div>
				<a href="{base}/hsi/classes" class="back-link">← Back to Classes</a>
				<h1>{classInfo.class_name}</h1>
			</div>
			<div class="header-actions">
				<a href="{base}/hsi/classes/{classInfo.class_code}/edit" class="btn-primary">Edit Class</a>
			</div>
		</header>
					...
					<tbody>
						{#each sessions as session (session.session_id)}
							<tr class:inactive={!session.is_active}>
								<td>
									<a href="{base}/hsi/classes/{classInfo.class_code}/sessions/{session.session_id}" class="session-link">{session.session_name}</a>
								</td>
