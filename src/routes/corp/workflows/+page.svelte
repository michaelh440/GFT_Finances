<!-- src/routes/corp/workflow/+page.svelte -->
<script>
  ...
  const CATEGORIES = [
    ...
    { key: 'pipeline_status', label: 'Pipeline Statuses' },
    { key: 'contract_status', label: 'Contract Statuses' },
  ];

  $: grouped = data.grouped ?? {};

  // ── Add form state ─────────────────────────────────────────────────
  let addCategory  = 'pipeline_status';
  let addValue     = '';
  let addLabel     = '';
  let addSortOrder = '';
...
<div class="container">
  <header>
    <div>
      <a href="/corp/engagements" class="back">← Back to Engagements</a>
      <h1>Corp Workflow Settings</h1>
      <p class="subtitle">Manage engagement types, pipeline statuses, and contract statuses</p>
    </div>
      ...
      <form method="POST" action="?/add" use:enhance={() => {
        ...
        <div class="add-grid">
          ...
          <label>Label <span class="hint">(shown in UI)</span>
            <input name="label" bind:value={addLabel} placeholder="e.g. Follow Up" />
          </label>
          <label>Value <span class="hint">(stored in DB)</span>
            <input name="value" bind:value={addValue} placeholder="e.g. follow_up" />
          </label>
          <label>Sort Order
            ...
            {:else}
              {#each rows as row (row.workflow_id)}
                <tr class:inactive={!row.is_active}>
                  <td>
                    {#if editingId === row.workflow_id}
                      <input class="inline-input"
                        bind:value={editValues[row.workflow_id].label}
                        placeholder="Label" />
                    {:else}
                  ...
                  <td>
                    <code class="value-code">{row.value}</code>
                  </td>
                  <td class="col-center">
                    {#if editingId === row.workflow_id}
                      <input class="inline-input sort-input" type="number"
                        bind:value={editValues[row.workflow_id].sort_order} />
                    {:else}
                      {row.sort_order}
                    {/if}
                  </td>
                  <td class="col-center">
                    {#if editingId === row.workflow_id}
                      <input type="checkbox"
                        bind:checked={editValues[row.workflow_id].is_active}
                        class="checkbox" />
                    {:else}
                      ...
                      <div class="row-actions">
                        ...
                        <form method="POST" action="?/delete" use:enhance={() => {
                          ...
                          <button type="submit" class="btn-delete"
                            on:click={(e) => {
                              if (!confirm(`Delete "${row.label}"? This cannot be undone if the value is in use.`))
                                e.preventDefault();
                            }}>
                            Delete
                          </button>
